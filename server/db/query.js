...
let Query = (function() {
    ...
    let getUser = (email, password, callback) => {
        ...
        connection.query(
            "SELECT email, username,id,builds FROM accounts WHERE email=? AND password=?;", [email, sha256(password)],
            (err, result) => {
                if (err) throw err;

                console.log(result[0]);

                callback(result[0] !== undefined ? { username: result[0].username, email: result[0].email, id: result[0].id, builds: result[0].builds } : undefined);
